...
impl RawTask {
    ...
    pub(crate) fn poll(self) {
        unsafe { (self.header().rawTaskVtable.poll)(self.headerPtr) }
    }

    pub(crate) fn dealloc(self) {
        unsafe { (self.header().rawTaskVtable.dealloc)(self.headerPtr) }
    }

    /// waker来自context.wake()函数
    pub(crate) unsafe fn try_read_output(self, dst: *mut (), waker: &Waker) {
        (self.header().rawTaskVtable.try_read_output)(self.headerPtr, dst, waker);
    }

    pub(crate) fn drop_join_handle_slow(self) {
        unsafe { (self.header().rawTaskVtable.drop_join_handle_slow)(self.headerPtr) }
    }

    #[cfg(feature = "sync")]
    pub(crate) unsafe fn finish(self, val_slot: *mut ()) {
        unsafe { (self.header().rawTaskVtable.finish)(self.headerPtr, val_slot) }
    }
}
...
pub(super) fn buildRawTaskVtable<T: Future, S: Schedule>() -> &'static RawTaskVtable {
    &RawTaskVtable {
        poll: poll::<T, S>,
        dealloc: dealloc::<T, S>,
